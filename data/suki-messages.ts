export interface SukiMessage {
  text: string;
  emoji: string;
}

export const sukiMessages = {
  greetings: [
  ...
  streak: [
    ...
    { text: "¡Cuidado con romper la racha de {n} días! Suki se pondrá triste... 😿", emoji: "🤔" },
  ...
  outOfLives: [
    { text: "¡Oh no, te has quedado sin vidas! 😴 Suki te sugiere repasar vocabulario para recuperar fuerzas.", emoji: "😴" },
  ],
  perfectLesson: [
...
export function getRandomMessage(category: keyof typeof sukiMessages, placeholderVal?: string | number): SukiMessage {
  const list = sukiMessages[category];
